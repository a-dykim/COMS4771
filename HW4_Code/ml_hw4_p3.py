...
def multi_GD_full_rotation(dist_mat, coord, max_totiter=1000, maxiter=1000, learning_rate=0.1, precision=1e-1, change_threshold=0.1):
    ...
    niter = 0
    changes = 1000 # random initial start
    while changes > change_threshold:
        ...
        for i in range(9):
            ...
            new[city2]= one_GD(int(dist_mat[city1][city2]), old[city1], old[city2], maxiter, learning_rate, precision)
...
def one_GD(dist12, coord1, coord2, maxiter=1000, learning_rate=0.1, precision=1e-2):
    """
    ...
    error = np.abs(initial_dist - dist12)
    c2_now = coord2.copy()
    c2_old = coord2.copy()
    niter = 0
    while error > precision and niter < maxiter:
        if niter == 0:
            grad_disc = (initial_dist - dist12) * (coord2- coord1) / initial_dist
...
result= multi_GD_full_rotation(distance_df,mockcity_dict, max_totiter=10000, maxiter=1000, learning_rate=0.01, precision=1e-6, change_threshold=1e-6)
